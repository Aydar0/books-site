from django.contrib.auth import get_user_model
from django.contrib.auth.models import Permission
from django.test import TestCase
from django.urls import reverse

from books.models import Book, Review


class BookTests(TestCase):

    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username='reviewuser',
            email='reviewuser@mail.ru',
            password='testpass123'
        )
        self.book = Book.objects.create(
            title="Harry Poter", 
            author="JK Rowling", 
            price="25.00"
        )
        self.review = Review.objects.create(
            book=self.book,
            author=self.user,
            review="An example review"
        )
        self.special_permision = Permission.objects.get(codename='read_all_books_status')

    def test_book_listing(self):
        self.assertEqual(f'{self.book.title}', 'Harry Poter')
        self.assertEqual(f'{self.book.author}', 'JK Rowling')
        self.assertEqual(f'{self.book.price}', '25.00')

    def test_book_list_view_for_logged_in_user(self):
        self.client.login(email='reviewuser@mail.ru', password='testpass123')
        response = self.client.get(reverse('book_list'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Harry Poter')
        self.assertTemplateUsed(response, 'books/book_list.html')

    
    def test_book_list_view_for_logged_out_user(self):
        self.client.logout()
        response = self.client.get(reverse('book_list'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, f'{reverse("account_login")}?next=/books/')
        response = self.client.get(f'{reverse("account_login")}?next=/books/')
        self.assertContains(response, 'Log In')

    def test_book_detail_view_for_with_permissions(self):
        self.client.login(email='reviewuser@mail.ru', password='testpass123')
        self.user.user_permissions.add(self.special_permision)
        response = self.client.get(self.book.get_absolute_url())
        no_response = self.client.get('/books/12345')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(no_response.status_code, 404)
        self.assertContains(response, 'An example review')
        self.assertContains(response, 'Harry Poter')
        self.assertTemplateUsed(response, 'books/book_detail.html')
